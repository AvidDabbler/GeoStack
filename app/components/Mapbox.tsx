...
export const BoundsUpdate = () => {
  ...
  const postBounds = async (bounds: number[][]) => {
    ...
    await axios.post(
      "/api/map/update-bounds",
      axios.toFormData(postData)
    );
  };
...
export function StopsLayer() {
  ...
  const getRoutesAndStops = async () => {
    ...
    mapbox.map.once("idle", async () => {
      ...
      mapbox.addSourceToState(MapSources.stopsSource, stopsSource as SourceDataType);
      mapbox.addLayerToState(
        LayerStyles.stopsLayerStyle,
        MapLayers.points
      );
      mapbox.addLayerToState(
  ...
  const getRoutes = async () => {
    try {
      ...
      mapbox.map.once("idle", async () => {
        ...
        mapbox.addSourceToState(MapSources.routesSource, routesSource as SourceDataType);
        if (mapbox.map.getLayer(MapLayers.stopsLayer))
          mapbox.addLayerToState(LayerStyles.routesLayerStyle, MapLayers.stopsLayer);
